...
**Scope**: Reproduce XFrame2D-style steel checks (NEN-EN 1993-1-1+C2+A1/NB:2016 nl) in a 8-crate Rust workspace, exposed via Tauri commands to the React UI, with OpenAEC §4.3 reporting.
**Source-of-truth references**:
- `verificatie calculations/original/Calc 2.pdf` — XFrame2D output, 3 beams (HEB160, HEB160, HFRHS200×200×16, S 235), 59 pages, full art. 6.2 + 6.3 + LTB-NB derivation
- `verificatie calculations/original/portal-frame.pdf` — XFrame2D output, 4 beams (UNP350, HEB160×2, HEB300, S 235)
- `verificatie calculations/Steel Properties.png` — SCIA-style Beam Properties dialog with EN 1993-1-1 tab

---

## 1. Goal & Acceptance Criteria

Build a complete, **systematically-applicable** EN 1993-1-1 steel check engine that reproduces — to 3 decimal places — every Unity Check value in the two reference XFrame2D calculations, presented in OpenAEC-styled reports with full step-by-step derivation matching the XFrame layout (option (a) from brainstorming).
...
- All 7 reference beams (3 from Calc 2, 4 from portal-frame) reproduce in automated Rust tests with `assert_relative_eq!` matching XFrame numbers to within 0.1% on resistances and 1% on UCs.
- Status verdicts (OK/NotOk) match XFrame exactly.
...
- `'en1993_calculations'` → per-beam page with full XFrame-style derivation
...
Order matches XFrame layout. Notes rendered inline (e.g., "V_z,Ed < V_z,pl,Rd / 2 — shear effect on moment can be neglected").
...
|------|-----------|--------|
| Resistance values | `max_relative = 1e-3` (0.1%) | XFrame shows 3 decimals |
| Unity checks | `max_relative = 0.01` (1%) | XFrame rounds to 2 decimals |
...
Each file: snippet from XFrame PDF → corresponding Rust test → explanation of any deviations (e.g. NB.157 √-rounding). Provides audit trail for structural-engineering review.
