...
**Goal:** Build an 8-crate Rust workspace that performs NEN-EN 1993-1-1+C2+A1/NB:2016 nl steel checks, exposed via Tauri commands to the React UI, that reproduces XFrame2D output for 7 reference beams (Calc 2: 3 beams + portal-frame: 4 beams) to within 0.1% on resistances and 1% on UCs.
...
**Verify-iterate loop is mandatory:** Phase 13 generates real PDF reports from the Tauri app and diffs them against the original XFrame PDFs. Iterate until match.
...
- [ ] Order matches XFrame: Compression, Bending decisive, Shear, M+V, M+N, LTB, 6.3.3 N+M, Deflection.
...
- [ ] Build BeamCheckInput from XFrame Calc 2 PDF: profile=HEB160, grade=S235, length=5.0 m, forces_envelope from PDF (3 force points: x=0 combo 2, x=2500 combo 2, x=2500 combo 1), lateral_bracing=empty, buckling_length=5.0 both axes, deflection_class=Floor 333.
...
- [ ] Read XFrame Calc 2 PDF page 53 for Beam 2 inputs (similar structure to Beam 1 but different forces). Use `pdftotext -layout` on calc2.pdf, extract section 2.6.2.
...
- [ ] Read XFrame Calc 2 PDF page 57 for Beam 3 (hollow section). Note: no LTB applies for closed hollow (M_b,Rd should equal M_c,Rd).
...
- [ ] Special attention for portal_beam1 (UNP350): channel section asymmetry affects LTB Mcr formula — verify Channel-specific S/C results match XFrame.
...
- [ ] Render structure XFrame-style: title + article (top right), force state line, KaTeX formula, variables expanded with arithmetic, UC formula with values, status badge, intermediate values list, notes (italic).
...
This is where we close the loop: build real test fixtures, run end-to-end, generate PDFs, diff against XFrame originals, fix any discrepancies, repeat until match.

### Task 13.1: Build calc2.femp fixture
Files: Create tests/fixtures/calc2.femp

- [ ] Open npm run tauri:dev. Build the Calc 2 model manually: 3 beams matching XFrame Calc 2 layout (read pages 2-4 of the PDF for exact node coords, beam connectivity, profiles, supports).
...
- [ ] Commit: test(fixtures) add calc2.femp matching XFrame Calc 2 model
...
- [ ] Commit: test(fixtures) add portal_frame.femp matching XFrame model
...
- [ ] Verify panel shows 3 beams with UC values matching XFrame:
  - Beam 1: UC about 1.06 (governing 6.2.5 bending)
  - Beam 2: UC value per PDF page 53
  - Beam 3: UC value per PDF page 57
- [ ] Open Report tab, scroll to EN 1993 Calculations section, verify per-beam derivation matches XFrame layout.
...
### Task 13.4: Generate PDF and compare against XFrame
...
- [ ] Document EVERY discrepancy in docs/verification/iteration-log.md with location, XFrame value vs Our value, hypothesized cause.
...
- [ ] Write a failing unit test in that crate using the XFrame value as expected.
...
- [ ] Commit per fix: fix(crate-name) align function-name with XFrame value-N (issue from calc2 iter 1)
- [ ] Re-generate PDF after each batch of fixes, new iteration: tests/output/calc2-iteration-N.pdf.

Continue until ALL 7 reference beams UC values match XFrame within tolerance (max_relative 0.01).
...
- [ ] For each of the 7 acceptance beams, create docs/verification/{calc2|portal}-beamN.md:
  ...
  - For each check (6.2.4, 6.2.5, ..., LTB, 6.3.3): XFrame value | Our value | Difference | Status
...
- Resistance values (kN/kNm): match XFrame to 0.1% (3 significant decimals)
- Unity checks: match to 1% (XFrame rounds to 2 decimals)
