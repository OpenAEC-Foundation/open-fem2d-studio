...
//! XFrame: Combination 2.1 (id=21) governs bending; 1.1 (id=11) for LTB.
...
fn run() -> &'static BeamCheckResult {
    ...
    RESULT.get_or_init(|| {
        let input = BeamCheckInput {
            ...
            // XFrame: Lcr,y=5000 mm, Lcr,z (not critical: chi_LT=1.0)
...
fn portal_beam4_bending() {
    let r = run();
    // XFrame: M_y,c,Rd = 439.199 kNm, UC = 0.62
...
fn portal_beam4_shear() {
    let r = run();
    // XFrame: V_c,z,Rd = 643.8 kN (Av=4745 mm²), UC = 0.36 (at x=5000 mm, Vz=-234.164 kN)
    ...
    // UC close to XFrame 0.36 (small delta due to profile DB Av_z: 4742 vs XFrame 4745 mm²)
...
fn portal_beam4_ltb_chi_is_one() {
    let r = run();
    // XFrame: lambda_LT = 0.236 < 0.4, so chi_LT = 1.00.
    ...
    // LTB UC should ~= bending UC when chi_LT=1.0 (XFrame: lambda_LT=0.236 < 0.4)
...
fn portal_beam4_governing_ok() {
    let r = run();
    // XFrame: all checks OK, UC_max = 0.62 (bending governs)
    assert!(r.uc_max < 1.0, "expected uc_max < 1.0 (XFrame: 0.62), got {}", r.uc_max);
