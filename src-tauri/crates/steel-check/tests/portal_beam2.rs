...
//!
//! Note: XFrame uses combination 2.1 (mapped to u32 21) and 2.2 (mapped to u32 22).
...
fn run() -> &'static BeamCheckResult {
    ...
    RESULT.get_or_init(|| {
        let input = BeamCheckInput {
            ...
            // XFrame: Lcr,y=2500 mm, Lcr,z=2500 mm
...
fn portal_beam2_compression() {
    let r = run();
    // XFrame: N_c,Rd = 1275.472 kN, UC = 0.18
...
fn portal_beam2_bending() {
    let r = run();
    // XFrame: M_y,c,Rd = 83.217 kNm, UC = 0.79
...
fn portal_beam2_shear() {
    let r = run();
    // XFrame: V_c,z,Rd = 239.1 kN, UC = 0.08
...
fn portal_beam2_governing_ok() {
    let r = run();
    // XFrame governing: 6.3.3 UC=0.79; beam is OK.
    // TODO Phase 13: assert governing_check_id == "6.3.3_n_my" once verified.
    assert!(r.uc_max < 1.0, "expected uc_max < 1.0 (XFrame: 0.79), got {}", r.uc_max);
