...
fn portal_beam1_compression() {
    let r = run();
    // Phase 13-G: UNP350 area aligned with XFrame (A=7665.7 mm²)
    // N_c,Rd = 7665.7 × 235 / 1.0 / 1000 = 1801.44 kN — matches XFrame exactly
...
fn portal_beam1_shear() {
    let r = run();
    // Phase 13-G: UNP350 Av_z aligned with XFrame (Av=4946 mm²)
    // V_c,z,Rd = 4946 × (235/√3) / 1.0 / 1000 = 671.06 kN — matches XFrame 671.1 kN
    assert_relative_eq!(resistance_value(r, "6.2.6_shear_z"), 671.06, max_relative = 1e-3);
    // UC = 235.084 / 671.06 = 0.350 (XFrame: 0.35)
