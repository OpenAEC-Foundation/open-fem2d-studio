// Het aanzicht op ware grootte (issue #45) op het echte tekenvlak, in de echte
// weergavelijst en in de echte constructieschets, in een lokale headless
// browser. De geometrie zelf bewaakt test-aanzicht-geometrie.mjs; deze test
// bewaakt dat die geometrie ook zo op het scherm komt: de ware hoogte rond de
// systeemlijn, de tekenvolgorde, de lijndikte bij zoom, het detail naar zoom,
// selecteren door de laag heen, de resultaatweergave, het thema, de schakelaar
// en het rapport. Het model is het startmodel van de app.
//
// Schermafbeeldingen: zet AANZICHT_SCHERMAFBEELDINGEN op een bestaande map en
// de test schrijft daar tekenvlak-licht.png, tekenvlak-donker.png,
// knoop-staal.png, betonbalk.png en rapport-schets.png. Zonder die variabele
// worden er geen bestanden gemaakt.
...
const folder = mkdtempSync(join(tmpdir(), "aanzicht-ui-"));
try {
  const bundle = await build({
    absWorkingDir: dirname(fileURLToPath(import.meta.url)),
    entryPoints: ["scripts/aanzicht-browser.tsx"], bundle: true, write: false,
  ...
  assert.equal(report.tests.filter((t) => t.error).length, 0, "Aanzicht-UI regressies");

  const map = process.env.AANZICHT_SCHERMAFBEELDINGEN;
  if (map) {
    assert.ok(existsSync(map), `map voor schermafbeeldingen bestaat niet: ${map}`);
    for (const [beeld, naam] of [["licht", "tekenvlak-licht.png"], ["donker", "tekenvlak-donker.png"], ["knoop", "knoop-staal.png"], ["beton", "betonbalk.png"], ["rapport", "rapport-schets.png"]]) {
